...
const AssignmentPreview = memo(({
  ...
  return (
    <div
      className={cn(
        isFullScreen ? "" : "border border-slate-200 rounded-md sm:rounded-xl md:rounded-2xl bg-slate-100",
        containerClasses
      )}
    >
      {isFullScreen && (
      ...
      <div className="w-full max-w-full sm:max-w-[90%] md:max-w-[85%] lg:max-w-3xl mx-auto space-y-5 sm:space-y-6 md:space-y-8 pt-4 px-4 sm:pt-6 sm:px-6 md:pt-8 md:px-8 lg:pt-[32px] lg:px-[36px] pb-6 lg:pb-[42px]">
        ...
        <PreviewField
          label="Type of work"
          value={`This artifact is a ${values?.subject || "unspecified"} project`}
        />
        ...
        <PreviewField label="Artifact Type" value={values?.artifact_type || "Not specified"} />
